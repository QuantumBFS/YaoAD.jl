...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=0.7\columnwidth,trim={0cm 0cm 0.2cm 0cm},clip]{images/vqe-bp.pdf}
    \end{center}
    \caption{A schetch of VQE algorithm, boxes are objectives and lines indicates data flow. (a) the quantum algorithm, (b) the classical simulation approach with Zygote, the blue lines are backward pass for calculating adjoint automatically.}\label{fig-vqe}
\end{figure}
...
\begin{align}
    ...
    \label{eq:heisenberg}
\end{align}
where $\<i,j \>$ denotes nearest neighbors pairs, $\sigmai{i}{\alpha=x, y, z}$ is a Pauli operator.

To obtain the gradient of parameters $\vtheta$ inside a quantum circuit, the quantum differentiable programming approach in \Fig{fig-vqe} (a) employs the relation~\cite{Mitarai2018}
...
In the classical simulation in \Fig{fig-vqe} (b), we are able to manipulate the wave functions directly, back propagation would help in decreasing the algorithm complexity to $O(N)$.

Simulation code combines Zygote and the package for quantum software 2.0 Yao.jl~\cite{Yao link}.
\begin{jcode}
using Yao
using YaoAD
using LinearAlgebra
